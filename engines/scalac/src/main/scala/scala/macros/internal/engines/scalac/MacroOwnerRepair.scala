...
class MacroOwnerRepair[C <: whitebox.Context with Singleton](val c: C) {
  ...
  def repairOwners[A](expr: c.Expr[A]): c.Expr[A] = {
    ...
    val typed = c
      .typecheck(c.universe.atPos(c.macroApplication.pos)(expr.tree))
      .asInstanceOf[symtab.Tree]
    ...
    def repairOwners(t: Tree, macroCallSiteOwner: Symbol): Tree = {
      object repairer extends Transformer {
        override def transform(t: Tree): Tree = {
          t match {
            case (_: DefTree | _: Function | _: Import)
                if t.symbol.owner == macroCallSiteOwner && macroCallSiteOwner != currentOwner =>
              new ChangeOwnerAndModuleClassTraverser(macroCallSiteOwner, currentOwner).apply(t)
            case _ =>
              super.transform(t)
          }
        }
...
object MacroOwnerRepair {
  def apply(ctxAny: Any, exprAny: Any): Any = {
    val c = ctxAny.asInstanceOf[whitebox.Context]
    val expr = exprAny.asInstanceOf[c.Expr[_]]
    c.Expr(c.untypecheck(expr.tree))
//    new MacroOwnerRepair[c.type](c).repairOwners(expr)
  }
}
